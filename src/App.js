  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(data.message || 'Failure occured uploading file.');
      }
    } catch (error) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-darkBlue">
      <div className="bg-darkPanel p-8 rounded-lg shadow-lg w-full max-w-lg text-center">
        <div className="border-dashed border-2 border-white p-6 rounded-lg">
          <svg className="mx-auto h-12 w-12 text-lightBlue" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16V4m4 12V4m4 12V4m0 4l-4-4-4 4m0 4l-4 4 4-4m4 4l4-4-4 4" />
          </svg>
          <p className="text-white mt-4">Attached File: {file ? file.name : 'None'}</p>
        </div>
        <input
          type="file"
